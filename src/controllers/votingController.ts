...
exports.postVote = async (req: express.Request, res: express.Response) => {
    try {
        const polyListData: any[] = [];
        const polyListId: string[] = [];
        const polySnap = await db.collection("PolymarketPredictions").get();
        polySnap.forEach((doc) => {
            polyListId.push(doc.id);
            polyListData.push(doc.data());
        });

        const blockchainData: any = {
            "CreatorAddress": "Px7195bab4-e23b-4cd3-b3fa-5ef2cbfefb25",
            "VotingToken": "PRIVI",
            "QuorumRequired": 0.5,
            "TotalVotes": 10000,

            "Hash": "c459d168478fd7b4b1e159e6206ee101e85588e0bdbd570e494a0795b5c117dd",
            "Signature": "0xdcd544170c3687169345d44047de2be0a4b67c60ac1f02975c6ce78f512bae556340ea4196ef9de76577e4f1ba8653d0c9e65381e145933ddd8ddbbe2e1aa3fe1b",

            "Caller": "PRIVI"
        }

        for (let i = 0; i < polyListId.length; i++) {
            const id = polyListId[i];
            let data = polyListData[i];
            blockchainData.VotationId = id;
            blockchainData.StartingDate = data.StartingDate;
            blockchainData.EndingDate = data.EndingDate;
            const blockchainRes = await votation.createVotation(blockchainData);
            if (blockchainRes && blockchainRes.success) {
                const output = blockchainRes.output;
                const updateVotation = output.UpdateVotations[id];
                const updateVotationStates = output.UpdateVotationStates[id];

                data.Tags = data.Category ? [data.Category] : [];
                data.Source = "Polymarket"
                if (data.StartingDate) delete data.StartingDate;
                if (data.EndingDate) delete data.EndingDate;

                const currAnswer: string[] = data.Answers;
                let newAnswer = [
                    {
                        AnswerId: 0,
                        Title: "No",
                ...
                if (!currAnswer.includes("No")) newAnswer = [
                    {
                        AnswerId: 0,
                        Title: "Mavericks",
                        Type: 'NO',
                ...
                data = {
                    ...data,
                    ...updateVotation,
                    ...updateVotationStates
                };
                db.collection(collections.growthPredictions).doc(id).set(data);
                console.log(id, data);
            }
            else console.log("error", blockchainRes.message);
        }
