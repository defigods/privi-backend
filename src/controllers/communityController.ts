...
exports.getTreasuryData = async (
  ...
  try {
    ...
    const blockchainRes = await coinBalance.getTokensOfAddress(
      communityAddress,
      apiKey
    );
    if (blockchainRes && blockchainRes.success) {
      ...
      tokenList.forEach((token) =>
        promises.push(coinBalance.balanceOf(communityAddress, token))
      );
    ...
    return (
      "Error in controllers/communitiesControllers -> getTreasuryData()" + e
    );
  }
};
