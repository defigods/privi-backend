...
module.exports.getTransfers = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    historySnap.forEach((doc) => {
      ...
      const data = { id: doc.data().Id, token: doc.data().Token, tokenRate: tokenRate, value: value, valueCurrency: valueCurrency, realValue: realValue, realValueCurrency: realValueCurrency, type: doc.data().Type, date: formatDate(date) };
      retData.push(data);
    })
    res.send({ success: true, data: retData });
  } catch (err) {
